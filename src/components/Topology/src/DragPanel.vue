...
<template>
  <transition name="slide-fade">
    <div 
      ref="ShapeContainer" 
      row-start-3 row-span-23 col-start-1 col-span-5 w-full h-full z-99
      ...
      <div h-full flex flex-col justify-between overflow-y-auto overflow-x-hidden>
        ...
        <div flex flex-col>
          ...
          <details 
            ...
            @toggle="toggleState($event, shape)" 
            :key="shape.id" 
            w-full
            class="group">
            <summary 
            ...
            <div 
              class="flex flex-wrap justify-start mx-2 mt-2 mb-3 min-h-20 transition-all duration-300 ease-out"
              :class="shape.status === 'open' ? 'opacity-100' : 'opacity-0'">
              <template v-if="shape.children">
                <div 
                  v-for="i in shape.children" 
          ...
          <details class="w-full group">
            <summary
              ...
              <span class="flex items-center justify-between font-medium">
                ...
                <i class="y-icon-park-solid:down-one align-middle transition-transform duration-300 ease-out group-open:rotate-180"></i>
              </span>
            </summary>
            <div class="min-h-70 mx-2 py-2 transition-all duration-300 ease-out">
              <details class="group/sub">
                <summary :class="['list-none px-4 py-2 text-neutral-500 cursor-pointer rounded-lg transition-all duration-300 ease-out hover:bg-white/5 hover:text-neutral-400']">test</summary>
                <details class="group/sub2">
                  <summary :class="['list-none px-6 py-1.5 text-neutral-500 cursor-pointer rounded-lg transition-all duration-300 ease-out hover:bg-white/5 hover:text-neutral-400']">subtitle</summary>
                </details>
              </details>
            </div>
          </details>
