...
<template>
    <transition name="slide-fade">
        <div row-start-3 row-span-23 col-start-1 col-span-5 w-full h-full z-99
           ...
           -->
                    <details v-for="(shape, index) in shapeList" @toggle="toggleState($event, shape)" :key="shape.id"
                        w-full>
                        ...
                        <div flex justify-start mx-2 m-t-2 min-h-20>
                            <template v-if="shape.children">
                                <div v-for="i in shape.children" w-10 h-10 bg-gradient-to-t from-dark-800 to-dark-300
                                    rounded flex justify-center items-center outline outline-1 outline-dark-100>
